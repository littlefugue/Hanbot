...
client.on('guildMemberAdd', member => {
  ...
  const hichannel = member.guild.channels.cache.find(ch => ch.name === '잡담');
  // Do nothing if the channel wasn't found on this server
  if (!hichannel) return;
  // Send the message, mentioning the member
  hichannel.send(`${member}, 한강디코에 오신 것을 환영합니다. 잡담방에 인사+실명 남겨주시고 공지사항은 전부 읽어 주세요. 관리자가 확인 후 등급이 부여됩니다.`);
});
